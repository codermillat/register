...
case "${1:-}" in
    ...
    reply)
        ...
        if ! command -v jq &> /dev/null; then
            echo "Error: jq is required for safe JSON escaping (install jq)"
            exit 1
        fi
        echo "Posting reply..."
        body=$(jq -nc --arg content "$content" '{content:$content}')
        api_call POST "/posts/${post_id}/comments" "$body"
        ;;
    create)
        title="$2"
        content="$3"
        # Moltbook currently expects submolt *name* (e.g. "general"), not UUID.
        submolt="${4:-general}"
        ...
        echo "Creating post..."
        body=$(jq -nc --arg title "$title" --arg content "$content" --arg submolt "$submolt" '{title:$title,content:$content,submolt:$submolt}')
